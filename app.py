...
<head>
    ...
    <title>AxtarGet – Video & MP3 Endirici</title>
    ...
    <style>
        ...
        *{margin:0;padding:0;box-sizing:border-box;}
        body{background:var(--bg);color:var(--text);font-family:'Inter',sans-serif;display:flex;min-height:100dvh;align-items:center;justify-content:center;padding:20px;transition:all .4s;overflow-x:hidden;background:radial-gradient(circle at 50% 50%,rgba(0,245,255,0.07),transparent 70%);}
        ...
        .card::before{content:'';position:absolute;inset:0;background:linear-gradient(135deg,rgba(0,245,255,0.1),rgba(255,0,170,0.1));opacity:0.5;pointer-events:none;}
...
<body>
    ...
    <div class="card">
        <h1>AxtarGet</h1>
        <p class="subtitle">Filigransız • 1080p • 1 kliklə</p>
    ...
    <script>
        ...
        function status(msg,cls='',spin=false){const s=document.getElementById('status');s.innerHTML=spin?'<div class="spinner"></div>'+msg:msg;s.className=cls;}
        async function download(){
            const url=document.getElementById('url').value.trim();
            if(!url){status('Link daxil et!','error');return;}
            status('Endirilir... 5-10 saniyə gözlə','loading',true);
            try{
                ...
                if(r.ok){
                    ...
                    a.click();
                    status('Uğurla endirildi! Növbətini göndər','success');
                    document.getElementById('url').value='';
                }else status('Xəta oldu, linki yoxla','error');
            }catch{status('Bağlantı xətası','error');}
        }
...
def download():
    ...
    if t == "music":
        opts.update({'format': 'bestaudio/best','postprocessors': [{'key': 'FFmpegExtractAudio','preferredcodec': 'mp3','preferredquality': '320'}],'outtmpl': 'out.%(ext)s'})
        final = "music.mp3"
    else:
    ...
    try:
        with yt_dlp.YoutubeDL(opts) as ydl:
            ...
            filename = "out.mp3" if t=="music" else ydl.prepare_filename(info)
        return send_file(filename, as_attachment=True, download_name=final)
